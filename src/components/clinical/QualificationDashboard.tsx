...
export function QualificationDashboard() {
  ...
  return (
    <div className="bg-background border border-border rounded-xl p-5 mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {geneEntries.map(({ key, data }) => (
          ...
          >
            ...
            <div
              className={`text-sm font-medium mt-0.5 transition-colors duration-150 ${data.qualified ? "text-tier-green" : "text-text-tertiary"}`}
            >
              {data.qualified ? "✅ Qualified" : "— Not covered"}
            </div>
            {data.medications.length > 0 && (
              <div className="text-[11px] text-text-secondary mt-1 leading-relaxed">{data.medications.join(", ")}</div>
            )}
          </div>
        ))}
      </div>
      ...
      {/* Row 2: Suggestions */}
      {/* Sprint 1: SuggestionCards disabled. Adding medications to "qualify"
          a gene before the patient's chart supports it caused the meloxicam-
          ghost class of error in v1. CPT selection must follow chart
          medications, never lead them. To re-add this UX safely, restore
          only after Sprint 2 (CPT-derived-from-medications gating) ships. */}
      ...
      {medications.length > 0 && (
        <div className="flex flex-col md:flex-row gap-4 mt-4">
          <div className="flex-1">
            ...
            <div
              className={`font-mono text-[11px] break-all rounded-md px-3 py-2 border ${
            ...
            <div
              className={`text-[11px] mt-1 ${box19.overLimit ? "text-destructive font-medium" : "text-text-tertiary"}`}
            >
          ...
          <div className="shrink-0">
            ...
            <div
              className={`text-xs mt-1 font-medium ${
                qualifiedCount === 0 ? "text-destructive" : panelEligible ? "text-primary" : "text-text-tertiary"
              }`}
            >
              {qualifiedCount === 0 ? "No billable tests" : panelEligible ? "Panel 81418 eligible" : "Single gene test"}
            </div>
          </div>
        </div>
      )}
      ...
      {(nonBillableMeds.length > 0 || yellowCodes.length > 0) && medications.length > 0 && (
        <div className="border-t border-border pt-3 mt-3 text-xs text-tier-yellow space-y-1">
          {nonBillableMeds.length > 0 && (
            <p>
              {nonBillableMeds.length} medication{nonBillableMeds.length > 1 ? "s" : ""} not billable:{" "}
              {nonBillableMeds.join(", ")}
            </p>
          )}
          {yellowCodes.length > 0 && (
            <p>
              {yellowCodes.length} ICD-10 code{yellowCodes.length > 1 ? "s" : ""} require LCD documentation:{" "}
              {yellowCodes.join(", ")}
            </p>
          )}
        </div>
