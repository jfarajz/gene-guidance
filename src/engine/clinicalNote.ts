import type { OrderState, Medication } from '@/types/order';
...
export function generateClinicalNote(state: OrderState): string {
  ...
  if (hasPsychDx) {
    ...
    if (consideredPsych.length > 0) {
      ...
      lines.push(`I want to consider alternative antidepressant therapy including ${medNames}. I want to use PGx testing for ${geneText} to evaluate the genetic impact on these medications and determine if the patient will respond appropriately to therapy.`);
      lines.push('');
    }
  }
  ...
  if (hasHTN) {
    ...
    for (const med of bbPrescribed) {
      lines.push(`The patient also has essential primary hypertension (${htnDx.code}) and is taking ${med.generic} ${med.dose}. I want to investigate the genetic impact of this medication through PGx testing of variants of the CYP2D6 gene and its impact on beta blocker therapy and ensure appropriate blood pressure control.`);
      lines.push('');
    }
    for (const med of bbConsidered) {
      lines.push(`The patient also has essential primary hypertension (${htnDx.code}) and is being considered for beta blocker therapy with ${med.generic}. I want to use PGx testing for the CYP2D6 gene to investigate the genetic impact on this cardiovascular medication.`);
      lines.push('');
    }
  }
  ...
  if (hasLipid) {
    ...
    if (currentStatin) {
      lines.push(`The patient has ${hasLipid.description} (${hasLipid.code}) and is currently on ${currentStatin.generic} ${currentStatin.dose}. I want to consider alternative statin therapy such as fluvastatin and use PGx testing for the CYP2C9 gene to evaluate the genetic impact on statin metabolism and determine optimal lipid management therapy.`);
    } else {
      lines.push(`The patient has ${hasLipid.description} (${hasLipid.code}). I want to use PGx testing for the CYP2C9 gene to evaluate the genetic impact on statin therapy such as fluvastatin for lipid management.`);
    }
    lines.push('');
  }
  ...
  if (hasClopidogrel) {
    if (hasClopidogrel.type === 'prescribed') {
      lines.push(`The patient is also taking clopidogrel ${hasClopidogrel.dose} for cardiovascular protection, which requires CYP2C19 testing to ensure proper activation of this prodrug.`);
    } else {
      lines.push(`Additionally, the patient may benefit from antiplatelet therapy with clopidogrel for cardiovascular protection. I want to use PGx testing for the CYP2C19 gene to assess the genetic impact on clopidogrel activation, as this prodrug requires proper metabolism for therapeutic efficacy.`);
    }
    lines.push('');
  }
  ...
  for (const med of nsaidMeds) {
    lines.push(`The patient is also taking ${med.generic} ${med.dose} for pain and inflammation management. I want to use PGx testing for the CYP2C9 gene to assess the genetic impact on this NSAID medication and ensure safe and effective therapy.`);
    lines.push('');
  }
  ...
  if (opioidMeds.length > 0) {
    ...
    lines.push(`The patient may also require pain management in the future, and CYP2D6 testing will help evaluate the genetic impact on opioid medications such as ${opioidNames}.`);
    lines.push('');
  }

  // Ondansetron
  if (ondansetron) {
    lines.push(`The patient may also require antiemetic therapy with ondansetron.`);
    lines.push('');
  }
  ...
  lines.push('');
  lines.push(`DR. ${provider.name.replace(/^Dr\.\s*/i, '').toUpperCase()}`);
